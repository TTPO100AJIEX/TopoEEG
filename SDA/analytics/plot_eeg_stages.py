...
def plot_eeg_stages(data: mne.io.Raw, edges: numpy.ndarray, epochs: mne.Epochs, resolution: float = 1) -> plt.Figure:
    ...
    for idx, (start, end, length) in enumerate(zip(edges_sec[:-1], edges_sec[1:], st_time_len)):
        ...
        ax.text(center, min + 1e-6, '{}s'.format(round(length)), fontsize = 9, fontstyle = 'italic', horizontalalignment = 'center') # Length
        ax.add_patch(ptchs.Rectangle((start, min - 3e-6), end - start, 3e-6, edgecolor = 'black', facecolor = color, fill = True, lw = 1)) # Stage
