...
Before examination, we re-reference the signals to the average reference and employ a bandpass filter at 0.9--40 Hz. Next, we apply independent component analysis and exclude the ECG, EOG, and T3 components corresponding to unrelated artifacts: breathing, heartbeat, blinking, etc. Finally, we split the EEG into 1-second epochs and discard the 10--15\% deviating most from the average power spectral density computed using the adaptive multitaper method, which is usually more stable than alternatives. Please refer to the original study~\cite{SDA} for details and specific pipeline parameters.
...
To speed up subsequent computations and enhance the stability, we remove the consistently least effective features based on previous results: Pearson dissimilarities; Betti curves and the number of points on persistence diagrams, including respective amplitudes; percentiles, skew, and kurtosis. This reduced the topological feature space to around 4,500 features (compared to 20,000 previously). Moving forward, we repeat all experiments for 30 subjects and report the results in Supplementary Materials D.

Based on the silhouette scores presented in Table D1 and summarized in Fig.~\ref{figure5}, topological features were generally comparable to, and in several cases outperformed, traditional features, whereas their combination yielded the best performance in approximately one third of the recordings. Consistent with previous observations, QSDA improved the results in most cases, further supporting its effectiveness for feature selection. In addition, Gaussian noise typically reduced the silhouette score by no more than a factor of three, whereas epoch shuffling produced substantially lower scores than the original EEG and, for most subjects, dropped the score below 0.05.
...
Exploring the detected boundaries also supports our previous assumptions that all feature sets identify similar boundaries, with only the least stable ones noticeably shifted by the framework. Interestingly, the clusters derived from topological features appeared quite dense for most subjects, suggesting that the new approach is not inferior in terms of stability. Combined features also produced more compact clusters than the other techniques, suggesting that the two feature sets not only support each other but may also detect complementary meaningful characteristics of the EEG.\@

Moreover, as shown in the right panels of Supplementary Materials D, the most consistently useful features were the amplitudes, especially those based on persistence silhouettes. Interestingly, the most informative data generally originated from frontal electrodes, as indicated by both QSDA and IV scores in Fig.~\ref{figure6}. However, this tendency was not uniform across subjects, with some showing the opposite pattern, underscoring the need for further investigation of subject-specific physiological variability.

Finally, on average, 27\% of topological features differ significantly between adjacent states in the original EEG, whereas none do in the shuffled data. Consistent with previous results, spectral validation in Table D2 confirms the pairwise significance of this observation. Together, these results support the framework's robustness and indicate that topological features reflect physiologically meaningful temporal structure.

Overall, the full-dataset results largely support the patterns observed in the initial analysis. Although some stage boundaries shifted when using topological features, main trends remained consistent, and robustness was supported by analyses of noisy and shuffled recordings. The proposed feature selection method also improved performance and was consistent with information values. Although further work is needed to clarify the physiological meaning of the findings and the sources of the observed discrepancies, we believe that the framework is a viable alternative to spectral features.

\subsection{Sleep data}

To reinforce our results and evaluate the applicability to other data, we ran the proposed algorithm on PSG recordings from the Sleep-EDF~\cite{SleepEdf,PhysioNet} dataset. For simplicity, this section analyzes three sets of features: spectral features similar to those in~\cite{SDA}, topological features, and a combination of both. Since the dataset contains only seven data channels, we omit feature selection. Other hyperparameters were not changed.

For numerical evaluation, we focus on two metrics: the adapted silhouette and~the Fowlkes-Mallows index (FMI)~\cite{Fowlkes01091983}, an external criterion that measures similarity between clusterings. As mentioned previously, in this experiment we search for 10~functional stages $A$ and estimate whether they match a best-fit subset $F$ of expert annotations $E$ with the lowest sum of pairwise distances: $\left( \sum_{i = 1}^{n} |a_i - f_i| \rightarrow_{F \subset E} \min \right)$. Although this matching procedure gives additional flexibility and may lead to optimistic estimates, the comparison is intended to assess structural correspondence rather than conventional sleep-staging accuracy. The reported FMI should thus be interpreted as an upper bound on structural agreement rather than as a classification score.

To provide finer temporal granularity, we primarily focus on 15-second epochs, half the conventional standard. This gives SDA a denser set of candidate transitions and reduces the risk of blurred boundaries. The experiment also highlights the potential for high-resolution analyses, including sleep fragmentation and insomnia research, where shorter epochs enhance sensitivity and reveal relevant patterns~\cite{aydn2011singular,aydn2011computer}.
...
Unlike meditation experiment, a slightly lower performance of the combined feature space suggests that topological descriptors also capture most of the information provided by traditional techniques. Moreover, this observation is highly significant, as revealed in the lower block of Table~\ref{table2}: both tests support superior quality of the topological approach, with large effect sizes and uniformly positive subject-level differences. The stronger result with shorter epochs is also consistent with finer temporal resolution and may reflect greater sensitivity to geometric changes in EEG structure.
...
\begin{figure} 
    ...
    \begin{minipage}[t]{0.625\textwidth} 
        ...
        \caption{Running time in seconds for calculating traditional and topological features from EEG with varying numbers of epochs. Each point is an average of 25 measurements on a standard 8-core Intel Ice Lake VM with 16 GB of RAM.\@ The 95\% confidence intervals for each entry do not exceed 1\% of their value}\label{figure8} 
    \end{minipage} 
\end{figure}
...
A major inherent limitation of QSDA is that both feature selection and final evaluation rely on the same SDA objective. Because the framework scores features individually rather than optimizing jointly and since it consistently agrees with independent IV ranking and statistical tests, we believe the risk of circular bias is likely reduced. Nevertheless, we acknowledge that a residual risk of inflated quality metrics remains. An important direction for future work is therefore to obtain unbiased effect estimates and provide stronger validation via a nested or a leave-one-out check.
