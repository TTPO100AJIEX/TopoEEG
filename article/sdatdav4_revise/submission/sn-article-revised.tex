...
\begin{figure}
    \centering
    \includegraphics[width=0.45 \columnwidth]{Fig1_revised.eps}
...
\begin{figure}
    \centering
    \includegraphics[width=0.63 \columnwidth]{Fig2_revised.eps}
...
In this study, we primarily work with Guhyasamaja Tantra meditation data. First, we focus on subjects 1 -- 3 and compare the results obtained using 765 traditional features from~\cite{SDA}, almost 20000 topological features, a subset of 1500 -- 2000 topological features selected with QSDA, and the feature space combining both approaches. At this stage, we determine the optimal hyperparameters, employ information value analysis (IV)~\cite{InformationValue}, and study QSDA scores to select approximately 4,000 topological features. The final hyperparameter values are listed in Supplementary Materials B. Then, we remove characteristics with consistently poor IV and QSDA quality from the computations and run experiments for all 30 subjects to ensure the reliability of the results. To reinforce the stability of the approach, we also report performance for noise-corrupted recordings, expecting no significant changes in the detected stages, and shuffled EEG epochs, anticipating no good boundaries detected, resulting in close-to-zero quality metrics. Additionally, to ensure statistical reliability of topological features, we use the non-parametric Mann--Whitney U--test with Bonferroni correction for p-values and report the proportions of statistically significant features with a confidence level of p-value $< 0.01$ for all pairs of identified stages.

\subsubsection{Full feature set for subjects~1 -- 3}

According to Fig.~\ref{figure3}, filtered topological features outperform traditional ones for all subjects across most choices of the number of PCA components. Additionally, the combined feature space achieves an even higher adapted silhouette score for subject 1, although following slightly behind for subjects 2 and 3. Notably, for all three subjects, QSDA significantly improved the quality of the resulting edges, indicating that the technique effectively filtered out noisy, harmful features. Moreover, as expected, SDA revealed no significant patterns when applied to the shuffled recordings, demonstrating a score more than 25 times lower than that of the original data. On the contrary, adding noise had a much smaller effect, indicating that the pipeline is relatively robust with respect to noise.

\begin{figure}
    \centering
    \includegraphics[width=0.9 \columnwidth]{Fig3_revised.eps}
...
Subsequently, the most practical features, consistently across QSDA and IV for all subjects, were the various persistence amplitudes. Interestingly, QSDA primarily favored those based on persistence silhouettes and landscapes, whereas those calculated from Betti curves received a higher IV.\@ Similarly, the most meaningful statistical characteristics were the vector norms, as well as the sum and mean of the sequences. The least informative, in turn, were all percentiles, skew, and kurtosis of variables. Aggregated by topological origin, although the features of dimensions 1 and 2 contributed the most, the best features were those describing all dimensions simultaneously, suggesting that higher dimensions do not duplicate but rather complement the patterns detected by simpler features.
...
\begin{figure}
    \centering
    \includegraphics[width=0.9 \columnwidth]{Fig4_revised.eps}
...
To further understand the contribution of different features to the final result, we perform an ablation study measuring the change of the detected boundaries when removing different groups of topological descriptors. As a measure of the difference between two partitions produced by SDA, we rely on the adjusted mutual information score, a common metric to compare clusterings. In Supplementary Material C, we show the loss of mutual information when removing the corresponding features for the three subjects. Notably, the criterion did not exceed 0.25 in most cases, indicating that the detected boundaries do not rely on individual descriptors but rather emerge from their combined behavior, which confirms the robustness of SDA with topological descriptors. Moreover, some groups showed a particularly minor influence on the final result, indicating that they describe redundant patterns already captured by other features.

Finally, the tables in Supplementary Materials C show that the proportion of statistically significant features between adjacent states (second diagonal) exceeds 10\% for most boundaries with an average of 30\% accross the three subjects, revealing considerable differences in the behavior of topological features between states. Importantly, no significant features were found for shuffled data, indicating that there is no temporal structure in this case, as expected. These results further support the conclusion that topological features capture meaningful temporal structure with high statistical reliability.

It is worth noting that the extremely high dimensionality of the topological features may have introduced some dispersion in the results. However, the proposed selection method (QSDA) demonstrated effectiveness in filtering out significant portions of harmful features, supported by information value analysis, an ablation study, and a statistical test. Although one may notice some discrepancies in its behavior compared to IV (for example, giving preference to data collected closer to the occipital brain region and overestimating the quality of some amplitudes), the principal patterns match for both approaches.
...
Finally, the statistical analysis of topological features reveals an average of 27\% of features that differ significantly between adjacent states in the original EEG, and none in the shuffled data. Similar to the first experiment, these results suggest that topological features successfully capture meaningful temporal patterns in the EEG and confirm the statistical relevance and robustness of the approach when applied to meditation data.
...
To assess the results, we focus on two primary quality metrics: the previously used adapted silhouette score and the Fowlkes-Mallows index (FMI)~\cite{Fowlkes01091983}. The latter is a typical external criterion used to measure the similarity between clusterings. For a predicted set of stage boundaries $A$ and an expert annotation $E$, we select a subset $F \subset E$ of boundaries with the lowest sum of pairwise distances to $A: \left( \sum_{i = 1}^{n} |a_i - f_i| \rightarrow \min \right)$ and compute the FMI to evaluate whether their `true' cluster distribution matches the predicted one.

Table~\ref{table1} presents the mean quality scores across the 153 subjects and shows that the topological approach is clearly superior to the traditional features for this dataset in both the adapted silhouette score and the FMI.\@ On average, the adapted silhouette is more than seven times higher for the topological features than for the traditional ones, corresponding to an approximately $29\sigma$ separation. Additionally, the predictions produced by the topological pipeline agree with the expert annotations on average 7\% better, as indicated by the FMI, with a $14\sigma$ advantage over the traditional features. These differences are not only consistent across subjects but also highly statistically significant, as suggested by the narrow confidence intervals for both metrics. Furthermore, a similar or slightly worse quality obtained with the combined feature space suggests that the topological approach supersedes traditional features, revealing not only new patterns but also supporting the already known ones.
...
\begin{tabular}{cccc}
  ...
  \midrule
    Traditional & $0.030 \pm 0.009$  & $0.843 \pm 0.017$ \\
    Topological & $0.228 \pm 0.010 $ & $0.911 \pm 0.013$ \\
    Combined &  $0.227 \pm 0.010 $ & $0.907 \pm 0.014$ \\
  \bottomrule
\end{tabular}
\end{table}
...
\begin{figure}
    \centering
    \includegraphics[width=0.7 \columnwidth]{Fig7_revised.eps}
