...
\author{
  ...
  \thanks{This work was supported by the Russian Science Foundation, Grant No. 24-68-00030. (Corresponding author: Aleksandr Abramov.)}
  \thanks{This work involved human subjects in its research. Approval of all ethical and experimental procedures and protocols was granted by the Bekhtereva Foundation University. The studies were conducted in accordance with the local legislation and institutional requirements.}
...
\begin{abstract}
The analysis of electroencephalograms (EEG) is primarily a manual, routine task performed exclusively by experts. Recent studies have addressed this issue and proposed the state-detecting algorithm (SDA) --- an automated technique to detect the functional stages with minimal human intervention using spectral features. This study aims to explore another aspect of the data --- the spatial characteristics of the EEG signals --- and introduces an alternative approach to feature engineering based on a novel field of topological data analysis (TDA). We propose an algorithm to extract a comprehensive description of almost 20,000 features and suggest qSDA --- a unique feature selection framework based on the performance of SDA. The proposed technique yielded well-separated stage boundaries for 30 EEG recordings of the Guhyasamaja meditation practice, suggesting that TDA complements the spectral features and reveals new, previously unknown patterns. Moreover, qSDA effectively filtered out the noisy features and appeared consistent with the information value analysis (IV), highlighting the brain regions that produce the most valuable data. The approach was also confirmed statistically stable, showing poor quality when applied to randomly shuffled epochs. The results prove the existence of significant patterns in the spatial structure of the studied signals, which may have interesting physiological interpretations leading to more robust functional stage detection.
\end{abstract}
...
The conclusive answer is decided by an expert decision accounting for the clustering quality metrics: the Ward's and centroid distances, the silhouette coefficient, the Calinski--Harabasz index, and the Davies--Bouldin index. In this study, we primarily focused on the silhouette coefficient that is calculated for one object using equation \eqref{eq1:C} and shows how similar this object is to other objects in its cluster ($C_x$) in comparison with objects in different clusters ($C_y$). The single value for the entire set is obtained by averaging the results across all studied objects. The metric value ranges from -1 to 1, where positive values indicate that the object is assigned to the correct cluster, which is well separated from other clusters.

\begin{subequations}
\label{eq1}
\begin{align}
    a(x, C_x) = {(|C_x| - 1)} ^ {-1} \sum_{y \in C_x} ||x - y||, \label{eq1:A}\\
    b(x, C_x) = \min_{C_y \ne C_x} {|C_y|} ^ {-1} \sum_{y \in C_y} ||x - y||, \label{eq1:B}\\
    s(x, C_x) = (b - a)~~/~~\max(a, b), \label{eq1:C}
\end{align}
\end{subequations}
...
Suppose $x=(x_1, x_2, \ldots, x_n)$ is a categorical feature with values from the set $\{ X_1, X_2, \ldots, X_k \}$ and $y=(y_1, y_2, \ldots, y_n)$ is a binary target variable. Then, for each value $X_i$, the weight of evidence is calculated using equation \eqref{eq2} as the ratio of the shares of positive and negative objects with this value of the feature. Finally, the information value of this feature is computed using equation \eqref{eq3} and is an estimate of how much the distribution of this feature coincides with the distribution of the target variable, thus showing how important this feature is for the correct classification of objects.
...
As illustrated in figure \ref{figure6}, topological features demonstrate results comparable to traditional ones, with some stage boundaries almost precisely matching for all types of features throughout all objects. Furthermore, according to Table~\ref{table1}, topological features outperform the traditional ones for all experiments, with the combined feature space achieving an even higher silhouette score for object~\mbox{--}~1, although following slightly behind for objects 2 and 3. 

However, the scatter plots in figure \ref{figure6} show noticeably higher density and better separation of the stage\mbox{-}2 boundary candidates when the spectral features are involved. This indicates higher stability of the final results obtained in those cases, except, arguably, for object~\mbox{--}~2, where the illustrations are insufficient to make conclusive decisions. Nevertheless, in all cases, SDA did not discover any meaningful patterns when applied to shuffled recordings and demonstrated over 10 times lower quality than for the original data, reinforcing the statistical significance of the results.

\begin{table}
\begin{center}
\caption{Silhouette coefficients of results obtained from original and shuffled recordings of all objects using three sets of features: traditional, topological, and combined.}
\label{table1}
...
The studies involving humans were approved by Bekhtereva Foundation University. The studies were conducted in accordance with the local legislation and institutional requirements. The participants provided their written informed consent to participate in this study.

\section{Acknowledgments}
\noindent This work was supported by the Russian Science Foundation, Grant No. 24-68-00030.
