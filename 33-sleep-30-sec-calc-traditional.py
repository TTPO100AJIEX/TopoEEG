...
def process(subj):
    ...
    annot_train = mne.read_annotations(f"{src_dir}/annotations.edf")
    annot_train.crop(annot_train[1]["onset"] - 15 * 60, annot_train[-2]["onset"] + 15 * 60)

    raw_data = mne.io.read_raw_edf(f"{src_dir}/eeg.edf", preload=True, infer_types=True, verbose='error')
    ...
    ft_dir_path = f"{dir}/traditional_features"
...
for i in tqdm.trange(0, 153):
    if os.path.exists(f'E:/CourseProject/sleep-edf/Subj{i}/exp_sleep_30_sec/traditional_features/df_ft_plv_ind_all.feather'):
        continue
    process(f"Subj{i}")
